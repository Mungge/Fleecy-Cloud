...
func (s *VMSelectionService) SelectOptimalVM(participant *models.Participant, criteria VMSelectionCriteria) (*VMSelectionResult, error) {
	...
	if criteria.MaxCPUUsage == 0 {
		criteria.MaxCPUUsage = 70.0 // Python 시뮬레이션과 동일
	}

	// 테스트용 기본값 설정 - 다양한 시나리오를 보기 위해
	if criteria.ModelSizeMB == 0 {
		criteria.ModelSizeMB = 500 // 기본값 500MB
	}
	if criteria.MinVCPUs == 0 {
		criteria.MinVCPUs = 1 // 최소 1코어
	}
	if criteria.MinRAM == 0 {
		criteria.MinRAM = 512 // 최소 512MB
	}
	if criteria.MinDisk == 0 {
		criteria.MinDisk = 5 // 최소 5GB
	}

	fmt.Printf("=== VM 선택 시작 (실제 데이터) ===\n")
	...
	for i, osVM := range openStackVMs {
		...
		// 모델 크기 기반 동적 리소스 체크 (Python 로직과 동일)
		requiredMemoryMB := int((float64(criteria.ModelSizeMB) * 2.0)) + 512 // 모델크기×2 + 0.5GB
		requiredDiskGB := int((float64(criteria.ModelSizeMB) / 1024.0 * 3.0)) + 1 // 모델크기×3 + 1GB
		
		if osVM.Flavor.RAM < requiredMemoryMB {
			fmt.Printf("  → 모델 크기 대비 RAM 부족으로 제외: %dMB < %dMB (모델:%dMB)\n", 
				osVM.Flavor.RAM, requiredMemoryMB, criteria.ModelSizeMB)
			continue
		}
		if osVM.Flavor.Disk < requiredDiskGB {
			fmt.Printf("  → 모델 크기 대비 Disk 부족으로 제외: %dGB < %dGB (모델:%dMB)\n", 
				osVM.Flavor.Disk, requiredDiskGB, criteria.ModelSizeMB)
			continue
		}

		// VirtualMachine 객체로 변환
		vm := VirtualMachine{
			InstanceID:    osVM.ID,
			Name:          osVM.Name,
	...
	// 4. Python 시뮬레이션과 동일한 우선순위 점수 기반 선택
