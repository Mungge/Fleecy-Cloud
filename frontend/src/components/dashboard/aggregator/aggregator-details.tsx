...
const AggregatorDetails: React.FC<AggregatorDetailsProps> = ({
  ...
  const [currentAggregator, setCurrentAggregator] = useState(aggregator);
  ...
  const progressPercentage =
    (currentAggregator.currentRound / currentAggregator.rounds) * 100;
  ...
  const handleControlAction = async (action: "pause" | "resume" | "stop") => {
    ...
    try {
      if (action === "pause") {
        await pauseAggregator(aggregator.id);
        setCurrentAggregator((prev) => ({
          ...prev,
          status: "pending" as const,
        }));
      } else if (action === "resume") {
        await resumeAggregator(aggregator.id);
        setCurrentAggregator((prev) => ({
          ...prev,
          status: "running" as const,
        }));
      } else if (action === "stop") {
        await deleteAggregator(aggregator.id);
        setCurrentAggregator((prev) => ({
          ...prev,
      ...
      console.log(
        `Action ${action} performed successfully on aggregator ${aggregator.id}`
      );
    } catch (error) {
      console.error(
        `Error performing ${action} on aggregator ${aggregator.id}:`,
        error
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">{currentAggregator.name}</h1>
            <div className="flex items-center space-x-2 mt-2">
              <Badge className={getStatusColor(currentAggregator.status)}>
                {getStatusText(currentAggregator.status)}
              </Badge>
              <Badge variant="outline">{currentAggregator.algorithm}</Badge>
              <Badge variant="secondary">
                {currentAggregator.cloudProvider}
              </Badge>
            </div>
          </div>
        </div>
        ...
        {/* Control Buttons */}
        {(currentAggregator.status === "running" ||
          currentAggregator.status === "pending") && (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">
                {currentAggregator.currentRound} / {currentAggregator.rounds}{" "}
                라운드
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <div className="text-lg font-bold">
                  {currentAggregator.participants}
                </div>
              </div>
              <div>
                <span className="font-medium">현재 정확도:</span>
                <div className="text-lg font-bold">
                  {currentAggregator.accuracy}%
                </div>
              </div>
              <div>
                <span className="font-medium">연합학습:</span>
                <div className="text-sm">
                  {currentAggregator.federatedLearningName}
                </div>
              </div>
              <div>
          ...
          <CardContent>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span>{currentAggregator.instanceType}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">리전:</span>
                <span>{currentAggregator.region}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">CPU:</span>
                <span>{currentAggregator.specs.cpu}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">메모리:</span>
                <span>{currentAggregator.specs.memory}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">스토리지:</span>
                <span>{currentAggregator.specs.storage}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">생성일:</span>
                <span>{formatDate(currentAggregator.createdAt)}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">마지막 업데이트:</span>
                <span>{formatDate(currentAggregator.lastUpdated)}</span>
              </div>
              {currentAggregator.cost && (
                <>
                  <div className="flex justify-between">
                    <span className="font-medium">현재 비용:</span>
                    <span className="font-bold">
                      {formatCurrency(currentAggregator.cost.current)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">예상 총 비용:</span>
                    <span className="font-bold">
                      {formatCurrency(currentAggregator.cost.estimated)}
                    </span>
                  </div>
                </>
              )}
